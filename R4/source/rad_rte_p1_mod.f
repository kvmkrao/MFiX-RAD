...
!                                                                      !
!  Module name: RADIATION P1 Model                                     !
!                                                                      !
!  Purpose:  solve the P1 Radiative Transfer Equation (RTE)            !
!                                                                      !
	...
	do L = 1,DIMENSION_BC
		if (.not.BC_DEFINED(L)) then
			...
			BC_HW(L) =  ew(L)/(2d0-ew(L))*1.5d0
                        BC_C(L) = ew(L)/(2d0-ew(L))*1.5d0 !*Sw(L) (Murali)
                        if (UNITS=='CGS') BC_C(L) = BC_C(L)*1.d3 ! sigma  value in CGS (Murali)  erg cm-2 K-4 s-1
		end if
	end do
	BC_W = ZERO
	BC_F = ZERO
...
!c-------------------------------------------------------------
    ...
    CALL SOURCE_PHI (S_P, S_C, ONES, G, 0, A_M, B_M)
...
!c-------------------------------------------------------------
    ...
    CALL ADJUST_LEQ (res1, LEQ_IT(9), LEQ_METHOD(9), LEQI, LEQM)
...
! Indices
      INTEGER :: I, J, K, I1, I2, J1, J2, K1, K2, IJK, &
                 IM, JM, KM
!--------------------------------------------------------------------//
      DOUBLE PRECISION :: BETAW,tmp,sbcon
...
! boundary) otherwise a mixed type boundary
                             if(bc_hw_t_g(L).eq.0.0) then
                              tmp = T_g(IJK)
                              A_M(IJK,east,M) = -HALF
                              A_M(IJK,0,M) = -HALF
                              B_M(IJK,M) = -4.0*sbcon*(tmp**4.0)
                             else
!-- rad_changes_begin                              
! interpolate beta at the boundary
                              tmp = bc_Tw_g(L) !T_g(IJK)
                              if(Tw(L).ne.UNDEFINED) tmp  = Tw(L)
                              ...
                              B_M(IJK,M) = -(BC_HW_PHI(L)*BC_PHIW(L)+&
                                       BC_C_PHI(L)*4.0*sbcon*(tmp**4.0))*BETAW
...
! interpolate beta at the boundary
                             if(bc_hw_t_g(L).eq.0.0) then
                              ...
                              tmp = bc_Tw_g(L) !T_g(IJK)
                              if(Tw(L).ne.UNDEFINED) tmp  = Tw(L)
                              if(BC_TYPE_ENUM(L).eq.MASS_INFLOW) tmp = T_g(IJK) 
      ...
      DO L = 1, DIMENSION_BC
         IF (BC_DEFINED(L)) THEN
            IF (BC_TYPE_ENUM(L)==P_OUTFLOW .OR. &
              ...
              ! write(*,*) "rte-p1", bc_hw_t_g(L), L, T_g(IJK), bc_Tw_g(L) !, I1, I2, J1, J2, K1, K2 
               I1 = BC_I_W(L)
               I2 = BC_I_E(L)
               J1 = BC_J_S(L)
               J2 = BC_J_N(L)
...
! modifications for cartesian grid implementation
      IF(CARTESIAN_GRID .AND. .NOT.(CG_SAFE_MODE(1)==1)) &
         CALL BC_G_CG(VAR, BC_PHIF, BC_PHIW, BC_HW_PHI, &
                        BC_C_PHI, M, A_M, B_M,BETA)
...
!                                                                      C
!^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^C
   SUBROUTINE BC_G_CG(VAR, BC_PHIF, BC_PHIW, BC_HW_PHI, &
      BC_C_PHI, M, A_M, B_M,BETA)
         ...
         IF(BLOCKED_CELL_AT(IJK).OR.ALONG_GLOBAL_GHOST_LAYER.AND.(.NOT.FLOW_AT(IJK))) THEN
            ...
            IF (CUT_CELL_AT(IP_OF(IJK))) THEN
               ...
               IF (BCT==CG_NSW.OR.BCT==CG_FSW.OR.BCT==CG_PSW) THEN
                  ...
                  ELSE 
                      !if(bc_hw_t_g(L).eq.0.0.or.BC_TYPE_ENUM(L).eq.p_outflow) then 
                      if(bc_hw_t_g(L).eq.0.0) then 
                        tmp = T_g(IJK)
                        A_M(IJK,east,M) = -HALF
                        A_M(IJK,0,M) = -HALF
