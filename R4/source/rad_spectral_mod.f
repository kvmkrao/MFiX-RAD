...
!            call spectralgasCalc(gasinfo, kg, emiss)
            call spectralgasCalc(gasinfo, kg, ai, emiss)
            spl = rad_spl
!            kg(1) =             
            k_g(ijk,:) =  -log(1.0d0 - kg) / spl  !kg(1)   
...
subroutine spectralgasCalc(gasinfo, kg, ai, emiss)
    use rad_spectral_gray, only :rad_spectral_graygas_calc,rad_spectral_graygasconst_calc
    use rad_spectral_gray, only :rad_spectral_graygaswsgg_calc, rad_spectral_nongraygaswsgg_calc
    implicit none
    type(gasInfoType), intent(in) :: gasinfo
    real(dp),dimension(nq), intent(out) :: kg
    real(dp),dimension(nq), intent(out) :: ai
