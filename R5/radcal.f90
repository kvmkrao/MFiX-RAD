subroutine radcal(wvmin,wvmAX,tbnd,TEMP, PRES, PLEN,AMEAN) 
!PROGRAM RADCAL
USE PRECISION_PARAMETERS
USE RADCAL_CALC
IMPLICIT NONE

 DOUBLE PRECISION, ALLOCATABLE, DIMENSION(:) :: TRANSMISSIVITY

 DOUBLE PRECISION :: AMEAN, PLANCK_MEAN_ABSORPTION, FLUX, TOTAL_LENGTH_M,             &
...
DOUBLE PRECISION :: TEMP,PRES, PLEN
DOUBLE PRECISION :: wvmin, wvmax, tbnd
...
TYPE ELEMENTS
   ...
   CHARACTER(2048)         :: COMMENTS
    DOUBLE PRECISION, POINTER, DIMENSION(:) :: BANDS
    DOUBLE PRECISION, POINTER, DIMENSION(:) :: TEMP_EXP
END TYPE ELEMENTS
...
! OPEN(UNIT=IO,FILE=TRIM(FILENAME),STATUS='UNKNOWN')
 ...
 OMMAX = wvmax
 TWALL = tbnd
 CALL READ_INPUT(TEMP, PRES, PLEN)
! CALL READ_INPUT(300.0d0, 1.0d0,0.3075d0)
 !CALL READ_INPUT(OMMIN,OMMAX,TWALL,TEMP, PRES,PLEN)
 CALL INIT_RADCAL
...
! WRITE(IO,*) 'TOTAL PATH LENGTH (M):',CHAR(9),  TOTAL_LENGTH_M
 ...
 WRITE(*,*) 'AMEAN (CM-1):', CHAR(9),CHAR(9),CHAR(9),  AMEAN
 WRITE(*,*) 'PLANCK MEAN ABSORPTION (CM-1):', CHAR(9), PLANCK_MEAN_ABSORPTION
 WRITE(*,*) 'TOTAL EMISSIVITY:', CHAR(9),CHAR(9), 1.0_EB-DEXP(-AMEAN*TOTAL_LENGTH_CM)
 WRITE(*,*) 'RECEIVED FLUX (W/M2/STR):', CHAR(9), FLUX
 WRITE(*,*) 'TOTAL TRANSMISSIVITY:', CHAR(9), CHAR(9),   TOTAL_TRANSMISSIVITY
 WRITE(*,'(A)') '-------------------------------------------------------------'
...
! VARIABLES PASSED IN
  DOUBLE PRECISION, DIMENSION(:), INTENT(IN) :: WAVE_LENGTH    ! WAVE LENGTH IN MICRON
  DOUBLE PRECISION, DIMENSION(:), INTENT(IN) :: TRANSMISSIVITY ! TRANSMISSIVITY

  DOUBLE PRECISION, INTENT(IN) :: PRESSURE
  DOUBLE PRECISION, INTENT(IN) :: PATH_LENGTH
...
!========================================================================================
SUBROUTINE READ_INPUT(TEMP, PRES,PLEN)
!========================================================================================
...
! LOGICAL           :: FILE_EXIST
DOUBLE PRECISION :: TEMP, PRES, PLEN 
!DOUBLE PRECISION :: OMMIN,OMMAX,TWALL
...
!TWALL = 500.0d0  
!CALL READ_POINT(300.0d0,1.0d0,0.3175d0)
CALL READ_POINT(TEMP, PRES, PLEN)
...
!==============================================================================
SUBROUTINE READ_POINT(T,PRESSURE,PLENGTH)
!==============================================================================
...
! POINTER GAS_PHASE SPECIES
 TYPE GAS_PHASE
    DOUBLE PRECISION, POINTER :: OBJ
    CHARACTER(LEN=30)      :: NAME
 END TYPE GAS_PHASE

 TYPE(GAS_PHASE), ALLOCATABLE, DIMENSION(:) ::  MOLE_FRACTION

 DOUBLE PRECISION :: SUM_MOLE_FRACTION

  
 DOUBLE PRECISION :: T, PRESSURE, PLENGTH
 DOUBLE PRECISION :: FV

 DOUBLE PRECISION, TARGET :: XCO2, XH2O, XCO, XCH4, XC2H4, XC2H6, XC3H8, &
 ...
 NAMELIST /PATH_SEGMENT/ T, PLENGTH, PRESSURE, XCO2, XH2O, XCO, XCH4, XC2H4, XC2H6, &
...
!    PLENGTH   = 0.3175 ! LENGTH OF THE SEGMENT IN METERS
!    PRESSURE = 1.0    ! PRESSURE IN ATM
    FV       = 0.0d0 ! SOOT VOLUME FRACTION
!     T        = 0.0_EB
!     PLENGTH   = 0.0_EB
!     PRESSURE = 0.0_EB
!     FV       = 0.0_EB
! INITIALIZE THE MOLE FRACTION OF SPECIES TO ZERO ON EACH NEW SEGMENT
     DO I_SPECIES_GAS = 1, N_RADCAL_SPECIES_GAS
        ...
        if(I_SPECIES_GAS.eq.2) MOLE_FRACTION(I_SPECIES_GAS)%OBJ = 0.2d0 
        if(I_SPECIES_GAS.eq.3) MOLE_FRACTION(I_SPECIES_GAS)%OBJ = 0.1d0 
        if(I_SPECIES_GAS.eq.13) MOLE_FRACTION(I_SPECIES_GAS)%OBJ = 0.7d0 
     ENDDO
...
! ENFORCE POSITIVE VALUES
     ...
     SEGMENT_LENGTH_M(I_SEGMENT)            = MAX(PLENGTH,   0.0_EB)
     TOTAL_PRESSURE_ATM(I_SEGMENT)          = MAX(PRESSURE, 0.0_EB)
     PARTIAL_PRESSURES_ATM(I_FV,I_SEGMENT)  = 0.0d0 !MAX(FV,       0.0_EB)
...
!  PARTIAL_PRESSURES_ATM(15,1) = 0.0*TOTAL_PRESSURE_ATM(1) ! CO
!  PARTIAL_PRESSURES_ATM(15,2) = 0.2*TOTAL_PRESSURE_ATM(2) ! H20 
!  PARTIAL_PRESSURES_ATM(15,3) = 0.1*TOTAL_PRESSURE_ATM(3) ! CO2 
!  PARTIAL_PRESSURES_ATM(15,4) = 0.0*TOTAL_PRESSURE_ATM(4) ! CH4 
!  PARTIAL_PRESSURES_ATM(15,5) = 0.0*TOTAL_PRESSURE_ATM(5) ! C2H4 
!  PARTIAL_PRESSURES_ATM(15,6) = 0.0*TOTAL_PRESSURE_ATM(6) ! C2H6 
!  PARTIAL_PRESSURES_ATM(15,7) = 0.0*TOTAL_PRESSURE_ATM(7) ! C3H8
!  PARTIAL_PRESSURES_ATM(15,8) = 0.0*TOTAL_PRESSURE_ATM(8) ! C3H6 
!  PARTIAL_PRESSURES_ATM(15,9) = 0.0*TOTAL_PRESSURE_ATM(9) ! C7H8 
!  PARTIAL_PRESSURES_ATM(15,10) = 0.0*TOTAL_PRESSURE_ATM(10) ! C7H16 
!  PARTIAL_PRESSURES_ATM(15,11) = 0.0*TOTAL_PRESSURE_ATM(11) ! CH3OH
!  PARTIAL_PRESSURES_ATM(15,12) = 0.0*TOTAL_PRESSURE_ATM(12) ! MMA 
!  PARTIAL_PRESSURES_ATM(15,13) = 0.7*TOTAL_PRESSURE_ATM(13) ! N2 
!  PARTIAL_PRESSURES_ATM(15,14) = 0.0*TOTAL_PRESSURE_ATM(14) ! CH4_OLD 
!  PARTIAL_PRESSURES_ATM(15,15) = 0.0*TOTAL_PRESSURE_ATM(15) ! O2 
